...
class Trainer:
    ...
    def train(self):
        ...
        while self.global_step < hparams['max_updates']:
            ...
            for batch in train_pbar:
                ...
                self.epoch_losses[-1]+=total_loss.cpu().item()
                if training_step % 100 == 0:
                    self.log_metrics({f'tr/{k}': v for k, v in losses.items()}, training_step)
                    plt.figure()
                    plt.plot(range(0,training_step,100),self.epoch_losses/100, marker='o')
                    plt.xlabel("Epoch")
                    plt.ylabel("Loss")
                    plt.title("Training Loss per Epoch")
                    plt.grid(True)

                    plot_path = os.path.join(self.work_dir, "training_loss.png")
    ...
    def test(self):
        model = self.build_model()
        optimizer = self.build_optimizer(model)
        self.global_step=load_checkpoint(model, optimizer, hparams['work_dir'])
        optimizer = None
